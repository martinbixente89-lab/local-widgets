...
export function base(el: HTMLElement, params: Record<string, unknown>, ctx: WidgetContext, className: string): HTMLElement { el.empty(); const root = el.createDiv({ cls: `local-widget ${className} local-widget-${color(params, ctx)}` }); root.dataset.size = text(params.size, 'medium'); if (ctx.editBlock) { const edit = root.createEl('button', { cls: 'local-widget-edit', attr: { 'aria-label': 'Modifier le bloc du widget' } }); edit.setText('✎'); edit.addEventListener('click', (event) => { event.stopPropagation(); ctx.editBlock?.(); }); } return root; }
